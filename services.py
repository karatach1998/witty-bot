...
from math_problems import MathProblem, MathTask, BookRef
...
class MathProblemsService:
    def __init__(self, subject_name):
        ...
        def load_tasks(filepath):
            return [
                ...
                for task in yaml.load(filepath.read_text())
            ]

        assert set(problems_dir.glob("*.yaml")) == set(problems_dir.iterdir())
        self._problems = {
            p.stem.split(maxsplit=1)[1]: load_tasks(p) for p in problems_dir.iterdir()
        }
        self._book = Book(BOOK_STORE_CONTAINER, **yaml.load(book_path.read_text()))
    ...
    def get_random_problem(self, chapter_title=None):
        ...
        return MathProblem(
            task=random_task.task, problem=random_problem, book_ref=random_task.book_ref
        )
    ...
    def get_solution_sequence(self, problem):
        ...
        for pod in res.results:
            try:
                yield (f"_WolframAlpha:_ {pod.title}", next(next(pod.subpod).img).src)
    ...
    def get_theory_pdf(self, problem, *, return_title=False):
        ...
        if start and end:
            ...
            chapter = next(
                filter(lambda ch: ch["title"] == chapter_title, self._book.chapters)
            )
...
class RussianRulesService:
    def __init__(self):
        ...
        self._russian_rules = RussianRules(**yaml.load(metainfo_path.read_text()))

    part_titles = property(lambda self: self._russian_rules.part_titles)

    def get_random_paragraph_html(self, part_title):
...
class BookCollectionService:
    def __init__(self):
        def load_book(path):
            return Book(BOOK_STORE_CONTAINER, **yaml.load(path.read_text()))

        self._books = OrderedDict(
            list(
                map(lambda b: (b.title, b), map(load_book, config.BOOKS_PATH.iterdir()))
            )
        )

    book_titles = property(lambda self: self._books.keys())

    def get_random_book_chapter_pdf(self, book_title, *, return_title_tuple=False):
